...
const { db, SBR_WS_ENDPOINT, updateDatabaseWithJobs, isInTable } = require('./jobCrud');
...
async function scrapeJobs() {
  ...
  try {
    ...
    const combinedJobs = [
      ...
      {
        employer: jobs[4].employer,
        city: jobs[4].city,
        startDate: jobs[4].startDate,
        shortCall: jobs[4].shortCall,
        jobClass: jobs[5].jobClass,
        positionsRequested: jobs[5].positionsRequested,
        positionsAvailable: jobs[5].positionsAvailable,
        book: jobs[5].book,
        worksite: jobs[5].worksite,
        hourlyWage: jobs[5].hourlyWage,
        reportTo: jobs[5].reportTo,
        requestDate: jobs[5].requestDate,
        comments: jobs[5].comments
      },
      // Add missing job entry for Local 77
      {
    ...
    // Filter jobs using table boundary validation
    const validJobs = combinedJobs.filter(isInTable);
    console.log(`Filtered ${combinedJobs.length - validJobs.length} invalid jobs`);
