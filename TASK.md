...
#### Checklist for lib\electrical_components\jj_snack_bar.dart

- [ ] Import ElectricalCircuitBackground and ElectricalNotificationType
- [ ] Modify _show method to wrap SnackBar in Stack with ElectricalCircuitBackground (opacity, componentDensity, copper border, transparent background)
- [ ] Update color coding for showSuccess, showError, showWarning, showInfo using AppTheme electrical colors
- [ ] Add electrical glow effect to box shadow
- [ ] Ensure shape uses RoundedRectangleBorder with appropriate borderRadius
...
#### Checklist for lib\electrical_components\jj_electrical_toast.dart

- [ ] Update toast container decoration to match demo (copper border, transparent background, glow effect)
- [ ] Verify ElectricalCircuitBackground integration in _ToastOverlay (opacity, componentDensity, enableCurrentFlow, enableInteractiveComponents)
- [ ] Update _ToastTheme configurations with AppTheme electrical colors
- [ ] Ensure _ElectricalProgressIndicator uses correct theme color
- [ ] Verify animation timings for entrance/exit effects
...
#### Checklist for lib\electrical_components\jj_electrical_notifications.dart

- [ ] Update ElectricalTooltip widget (copper border, transparent background, color coding)
- [ ] Enhance tooltip decoration (border width, background color, glow effect)
- [ ] Update ElectricalCircuitBackground integration (opacity, componentDensity, disable current flow and interactive components)
- [ ] Add contextual tooltip messages based on mood
- [ ] Ensure _SparkEffectPainter provides appropriate visual feedback
...
#### Checklist for lib\design_system\components\reusable_components.dart

- [ ] Update JJSnackBar class to delegate to electrical-themed version (imports)
- [ ] Modify showSuccess method to use showElectricalSnackBar (type: success, maintain signature)
- [ ] Modify showError method to use showElectricalSnackBar (type: error)
- [ ] Modify showInfo method to use showElectricalSnackBar (type: info)
- [ ] Add comment at top of JJSnackBar class about delegation and features
...
#### Checklist for lib\screens\onboarding\welcome_screen.dart

- [ ] Locate button rendering logic in bottom navigation
- [ ] Identify button text condition
- [ ] Modify JJPrimaryButton to conditionally reduce font size on third page
- [ ] Implement reduced font size calculation and custom text style
- [ ] Ensure button functionality and styling unchanged
- [ ] Test font size reduction applies only to "Get Started" button
...
#### Checklist for lib\screens\onboarding\auth_screen.dart

- [ ] Locate SegmentedTabBar widget
- [ ] Identify height mismatch
- [ ] Fix alignment by adjusting heights (recommended Option A)
- [ ] Update animated indicator container height
- [ ] Ensure tab buttons fill entire height (verify TextButton, remove padding)
- [ ] Test alignment of bottom borders
- [ ] Verify gradient and shadow effects after adjustment
...
#### Checklist for lib\screens\onboarding\onboarding_steps_screen.dart

- [ ] Implement Step 1: Replace notifications with electrical toasts, ensure save and navigation
- [ ] Implement Step 2: Replace notifications with electrical toasts, ensure save and navigation
- [ ] Implement Step 3: Replace notifications with electrical toasts, ensure save and navigation
- [ ] Enhance error handling with specific messages and warning toasts
- [ ] Ensure loading state with _isSaving and indicator
- [ ] Update imports for JJElectricalNotifications
...
#### Checklist for lib\screens\home\home_screen.dart

- [ ] Apply Title Case formatting to all displayed text
- [ ] Add two dummy containers in Quick Actions (add cards, adjust layout)
- [ ] Remove colored fonts from cards and dialogs (review methods, remove grey tint)
- [ ] Clean up job cards display (spacing, verify updated widget)
- [ ] Ensure dialog popups apply Title Case and remove colored fonts
...
#### Checklist for lib\widgets\condensed_job_card.dart

- [ ] Remove grey tint on local number (change background, keep copper border)
- [ ] Remove colored fonts (remove conditional coloring, use neutral colors)
- [ ] Verify Title Case formatting (ensure all text through formatter)
- [ ] Update RichText styling (bold labels, regular values, remove overrides)
- [ ] Maintain copper border and divider
...
#### Checklist for lib\screens\jobs\jobs_screen.dart

- [x] Apply Title Case formatting to filters and labels
- [x] Verify job card integration (proper data, updated widget)
- [x] Update error messages to Title Case
- [x] Update filter chips to Title Case
- [x] Update imports if needed
- [x] Maintain electrical theme (background, copper elements)
...
#### Checklist for lib\widgets\rich_text_job_card.dart

- [x] Remove grey tint on local number
- [x] Add copper divider between sections
- [x] Remove colored fonts (remove conditional, no colored text)
- [x] Improve card spacing for even distribution
- [x] Enforce left-to-right, top-to-bottom data flow (verify layout)
- [x] Implement RichText with proper spans (bold labels, regular values)
- [x] Minimize N/A values with better formatting (null-safes, fallbacks)
- [x] Apply Title Case formatting (verify all values)
...
#### Checklist for lib\screens\storm\storm_screen.dart (Main Modifications)

- [x] Remove "Current Storm Activity" section
- [x] Remove "Active Storm Events" section
- [x] Update emergency work available container color to navy/copper gradient
- [x] Button up storm contractors card (ensure styling, error handling)
- [x] Implement PowerOutage.us API integration with toggle/accordion
- [x] Set up foundation for real-time emergency declaration videos (admin only)
- [x] Maintain power outage section functionality
...
#### Checklist for lib\screens\storm\storm_screen.dart (Toggle/Accordion Additions)

- [ ] Add state variable for toggle
- [ ] Create toggle header (row, icons, title, subtitle, tappable)
- [ ] Implement accordion behavior (AnimatedContainer, show/hide cards, animation)
- [ ] Sort states for easy viewing (add secondary sort, toggle/dropdown)
- [ ] Add state filter/search (TextField, onChanged filter)
- [ ] Enhance loading and error states (indicator, handling)
- [ ] Update notification usage to electrical toasts
...
#### Checklist for lib\features\crews\screens\tailboard_screen.dart (Header Updates)

- [ ] Replace welcome header with compact crew header (reduce sizes, padding)
- [ ] Fix horizontal overflow error (wrap in Flexible, add ellipsis, test landscape)
- [ ] Optimize quick stats row (wrap in Flexible/Expanded, adjust for landscape)
- [ ] Maintain existing functionality (dropdown, menu, stats, tab bar)
...
#### Checklist for lib\features\crews\screens\tailboard_screen.dart (Dialog Methods)

- [ ] Implement _showCreatePostDialog (dialog content, submit logic, styling)
- [ ] Implement _showShareJobDialog (dialog content, submit logic, styling)
- [ ] Implement _showNewMessageDialog (member selection, navigation)
- [ ] Update FAB logic (ensure calls to dialogs per tab)
- [ ] Add error handling (try-catch, electrical toasts)
...
#### Checklist for lib\features\crews\screens\crew_onboarding_screen.dart

- [ ] Remove Join a Crew button (delete widget, remove spacing)
- [ ] Change Create a Crew button text and icon
- [ ] Update layout for full-width button and centering
- [ ] Maintain animations (fade-in, smooth timing)
- [ ] Update screen description to focus on crew creation
...
#### Checklist for lib\features\crews\screens\create_crew_screen.dart

- [ ] Add all classifications (import enum, replace hardcoded items, Title Case)
- [ ] Add copper borders around all input fields (TextFormField, dropdown)
- [ ] Add electrical circuit background (wrap in Stack)
- [ ] Change switch to JJ Circuit Breaker Switch (replace, import, implement Row)
- [ ] Add persistent animation/gradient/shadow to Create Crew button (AnimatedContainer, pulsing glow)
- [ ] Maintain consistency with app design schema (use AppTheme constants)
...
#### Checklist for lib\features\crews\widgets\crew_preferences_dialog.dart

- [ ] Fix overflow on top right corner (wrap title, fixed constraints, ellipsis, test)
- [ ] Replace job types with construction types (remove classifications, import enum, update list, title, description)
- [ ] Update FilterChip implementation (rename method, update Wrap, change preferences model)
- [ ] Change save button text (keep styling/loading)
- [ ] Fix permission error when saving (add error handling, verify roles, TODO for rules)
- [ ] Update CrewPreferences model (add field, update copyWith/toMap/fromMap)
...
#### Checklist for lib\features\crews\widgets\tab_widgets.dart (Feed Tab)

- [ ] Update access control (allow all users, check currentUser, show global/crew feed)
- [ ] Add post creation FAB/input window (implement dialog, use notifier)
- [ ] Display posts with 50-post limit (update query, sort, ListView)
- [ ] Auto-populate prior posts (stream, RefreshIndicator)
- [ ] Style with electrical theme (PostCard colors, background, borders)
...
#### Checklist for lib\features\crews\widgets\tab_widgets.dart (Chat Tab)

- [ ] Implement crew members only access (check membership, show message)
- [ ] Design chat box with proper colors (replace placeholder, ListView, fetch messages, MessageBubble)
- [ ] Implement message alignment (right for user, left for incoming)
- [ ] Newest messages at bottom (sort chronological, auto-scroll)
- [ ] Single line text input with send button (add ChatInput, wire callback)
- [ ] Implement circular avatar with initials (use CrewMemberAvatar, fetch name)
- [ ] Style with electrical theme (bubbles colors, copper accents)
...
#### Checklist for lib\features\crews\widgets\tab_widgets.dart (Members Tab)

- [ ] List all members in active crew (replace placeholder, fetch, ListView)
- [ ] Display member information (name, local, books, preferences, use CrewMemberListItem)
- [ ] Implement member selection (onTap, show bottom sheet with options)
- [ ] Create member profile view (show details, modal/sheet, styling)
- [ ] Integrate with direct messaging (navigate, set up conversation)
- [ ] Use CrewMemberAvatar for consistent display (import, use for avatars)
- [ ] Style with electrical theme (copper borders, background, spacing)
...
#### Checklist for lib\domain\enums\enums.dart

- [ ] Review current classifications
- [ ] Add additional classifications if needed (cableSplicer, groundman, apprentice)
- [ ] Ensure Title Case display (use toTitleCase in UI)
- [ ] Verify ConstructionTypes enum (no changes)
- [ ] Update the all getter (ensure correct, apply Title Case in UI)
...
#### Checklist for lib\features\crews\models\crew_preferences.dart

- [ ] Add or update construction types field (replace/supplement jobTypes)
- [ ] Update constructor (add parameter, backward compatibility)
- [ ] Update fromMap/fromFirestore (parsing, migration)
- [ ] Update toMap/toFirestore (serialization, optional jobTypes)
- [ ] Update copyWith method (add parameter)
- [ ] Add validation (use ConstructionTypes.all)
...
#### Checklist for lib\features\crews\services\crew_service.dart

- [ ] Update crew creation method (accept/save constructionTypes, validate)
- [ ] Update crew update method (handle updates, add permission check)
- [ ] Fix permission error (review write op, error handling, logging)
- [ ] Add migration logic (from jobTypes to constructionTypes)
- [ ] Update job matching logic (use constructionTypes, update filters/queries)
...
#### Checklist for lib\widgets\dialogs\job_details_dialog.dart

- [ ] Apply Title Case to all text (import, apply to fields, use formatter)
- [ ] Remove colored fonts (review Text, remove conditionals, use neutral colors)
- [ ] Ensure consistent styling (RichText, copper borders, spacing)
- [ ] Update dialog decoration (electrical styling, copper border, background)
...
#### Checklist for lib\widgets\storm\power_outage_card.dart

- [ ] Verify current implementation (display, tappable, styling)
- [ ] Ensure proper data display (state, count, severity, percentage)
- [ ] Add expandable details (affected utilities, restoration time, locals, ExpansionTile)
- [ ] Optimize for list display (margins, spacing, tap targets)
- [ ] Maintain electrical theme (copper borders, color coding, text)
...
#### Checklist for lib\widgets\contractor_card.dart

- [ ] Apply Title Case to company name (verify line 45)
- [ ] Remove colored fonts (review Text, use neutral colors)
- [ ] Verify copper border (ensure width)
- [ ] Ensure proper button styling (keep consistent)
- [ ] Maintain electrical theme (border, shadows, integration)
...
#### Checklist for lib\features\crews\widgets\post_card.dart

- [ ] Verify electrical theme styling (background, border, colors)
- [ ] Update notification usage (replace SnackBar with toasts, import, types)
- [ ] Ensure avatar integration (CrewMemberAvatar, initials, size)
- [ ] Verify comment functionality (integration, fetch, actions)
- [ ] Maintain interaction features (like animation, reaction, bookmark, feedback)
...
#### Checklist for lib\features\crews\widgets\message_bubble.dart

- [ ] Verify message alignment (right/left based on user)
- [ ] Verify avatar display (left/right, CrewMemberAvatar, initials)
- [ ] Verify color coding (_getBubbleColor, system/job/user/incoming)
- [ ] Verify circuit background (CircuitPatternPainter, subtle opacity)
- [ ] Ensure proper text contrast (_getTextColor, readable)
- [ ] Maintain electrical theme (copper borders, glow, consistent styling)
...
#### Checklist for lib\features\crews\widgets\chat_input.dart

- [ ] Add copper border to input field (update decoration, focus state glow)
- [ ] Update send button styling (vibrant copper, glow/pulse)
- [ ] Verify keyboard activation (textInputAction, onSubmitted)
- [ ] Maintain electrical theme (backgrounds, shadow, elevation)
- [ ] Add character counter (maxLength, counterText or custom)
...
#### Checklist for lib\features\crews\widgets\crew_member_avatar.dart

- [ ] Verify initials display (extract logic, handle edge cases)
- [ ] Verify avatar customization (URL display, fallback to initials)
- [ ] Verify circular shape (borderRadius half size)
- [ ] Verify online status indicator (colors, position, size)
- [ ] Verify copper border (color, width)
- [ ] Verify CrewMemberListItem (display info, onTap, use in Members tab)
- [ ] Maintain electrical theme (copper accents, shape, sizing)
...
#### Checklist for lib\features\crews\services\message_service.dart

- [ ] Create MessageService class (imports, models, services)
- [ ] Implement send message method (signature, create document, fields, return ID, errors)
- [ ] Implement get messages stream (signature, query, map, filter, return stream)
- [ ] Implement direct message methods (sendDirectMessage, conversation ID, store)
- [ ] Implement get direct messages stream (signature, generate ID, query)
- [ ] Integrate with ChatService (mark read/delivered, status updates)
- [ ] Add error handling (try-catch, exceptions, logging)
...
#### Checklist for lib\features\crews\providers\messaging_riverpod_provider.dart

- [ ] Verify MessageService provider (returns instance)
- [ ] Verify crew messages stream provider (fetches, chronological)
- [ ] Verify direct messages stream provider (conversation ID, integrates)
- [ ] Add send message notifier (create, method, loading/errors, feedback)
- [ ] Verify unread count providers (count, real-time)
- [ ] Add helper providers (typed, correct data, error handling)
...
#### Checklist for lib\features\crews\widgets\member_profile_dialog.dart

- [ ] Create MemberProfileDialog widget (parameters)
- [ ] Design dialog layout (header, info, preferences, metadata)
- [ ] Add action buttons (send message, close)
- [ ] Style with electrical theme (border, background, colors, spacing)
- [ ] Handle missing data (null-safe, "Not specified")
- [ ] Add loading state (indicator, electrical-themed)
...
#### Checklist for lib\features\crews\widgets\create_post_dialog.dart

- [ ] Create CreatePostDialog widget (Stateful, parameters)
- [ ] Design dialog layout (header, content, footer)
- [ ] Add electrical circuit background (Stack, opacity, density)
- [ ] Implement form validation (not empty, length, error toast)
- [ ] Implement submit logic (validate, call onSubmit, loading, close, error)
- [ ] Add media upload placeholder (button, message, TODO)
- [ ] Style with electrical theme (borders, colors, animations, spacing)
...
#### Checklist for lib\widgets\storm\power_outage_toggle_header.dart

- [ ] Create PowerOutageToggleHeader widget (parameters)
- [ ] Design header layout (container, padding, row, icons, title, subtitle)
- [ ] Add tap interaction (InkWell/GestureDetector, callback, ripple)
- [ ] Add animation (icon rotation, duration, curve)
- [ ] Style with electrical theme (border, background, shadow, spacing)
- [ ] Add accessibility (semantic label, tap target, haptic)
...
#### Checklist for lib\widgets\storm\emergency_video_placeholder.dart

- [ ] Create EmergencyVideoPlaceholder widget (parameters)
- [ ] Design placeholder layout (container, header, content)
- [ ] Add commented-out video player code (TODO comments, example)
- [ ] Add admin upload placeholder (commented-out code, flow)
- [ ] Add access control placeholder (commented-out admin check)
- [ ] Style with electrical theme (border, background, colors, spacing)
- [ ] Add placeholder content (message, icon, admin message, badge)
...
#### Checklist for lib\utils\text_formatting_wrapper.dart

- [ ] Verify toTitleCase function (handles cases)
- [ ] Add enum formatting helper (formatEnumName, insert spaces)
- [ ] Add classification formatter (formatClassificationName)
- [ ] Add construction type formatter (formatConstructionType)
- [ ] Update JobDataFormatter class (use toTitleCase, add missing)
- [ ] Add null safety (handle nulls, defaults)
...
#### Checklist for firebase\firestore.rules

- [ ] Add or update crew preferences rules (allow update for admin/foreman, read for members)
- [ ] Add construction types validation (list, hasAll valid values)
- [ ] Ensure backward compatibility (allow jobTypes/constructionTypes)
- [ ] Add logging for debugging (track denials)
- [ ] Test rules (emulator, roles, ensure access)
- [ ] Document rules (comments, user roles, examples)
...
#### Checklist for lib\electrical_components\jj_electrical_tooltip_wrapper.dart

- [ ] Create JJElectricalTooltipWrapper widget (parameters)
- [ ] Implement contextual tooltip messages (helper method, symbols)
- [ ] Implement mood-based styling (language, glow colors)
- [ ] Build method (enabled check, wrap with electricalTooltip)
- [ ] Add convenience methods (static for types)
- [ ] Export from electrical components (add to file if exists)
...
#### Checklist for lib\features\crews\widgets\member_selection_bottom_sheet.dart

- [ ] Create MemberSelectionBottomSheet widget (parameters)
- [ ] Design bottom sheet layout (header, action list)
- [ ] Implement action handlers (ListTile, icons, text, tap/close/callback)
- [ ] Style with electrical theme (background, accents, shadow, spacing)
- [ ] Add animations (slide up, fade out, ripple)
- [ ] Add accessibility (labels, tap targets, feedback)
...
#### Checklist for lib\features\crews\widgets\share_job_dialog.dart

- [ ] Create ShareJobDialog widget (Stateful, parameters)
- [ ] Design dialog layout (header, content, footer)
- [ ] Implement job selection (fetch, display, search/filter)
- [ ] Implement share logic (validate, create post, type, metadata, callback, toast, close)
- [ ] Add electrical circuit background (Stack, opacity)
- [ ] Style with electrical theme (borders, colors, spacing, buttons)
